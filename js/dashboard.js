...
(function ($) {
    ...
    $(function () {
        ...
        if ($("#performaneLine").length) {
            ...
            // Initialisation des données du graphique avec des valeurs vides
            var salesTopData = {
                labels: [],  // Les données seront mises à jour dynamiquement
                datasets: [{
            ...
            async function fetchCryptoData() {
                try {
                    ...
                    const cryptoData = await getHistoricalData(symbol, currency, days);

                    if (cryptoData !== null) {
                        // Mettez à jour les données du graphique
                        salesTop.data.labels = cryptoData.map(entry => moment(entry.timestamp).format('HH:mm'));
                        salesTop.data.datasets[0].data = cryptoData.map(entry => entry.price);

                        // Mettez à jour le graphique
                        salesTop.update();
                    }
                } catch (error) {
                    console.error('Erreur lors de la récupération des données de l\'API CoinGecko:', error);
                }
            }
            ...
            // Mettez en place une minuterie pour mettre à jour les données toutes les X secondes (par exemple, toutes les 60 secondes)
